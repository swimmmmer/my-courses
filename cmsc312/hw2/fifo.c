...
int* buffer_index;
int segment_id, segment_id1, segment_id2, segment_id3, segment_id4, segment_id5, segment_id6, segment_id7, segment_id8, segment_id9;
int* producers_done;
int num_prod;
double total_wait;
int* num_elements;
...
void insertbuffer(print_job_t value) {
    if (*num_elements == SIZE) {
        ...
        *(buffer + *in) = value; 
        *num_jobs_produced = *num_jobs_produced + 1;
...
void *consumer(void *thread_n) {
    ...
    while (*producers_done != num_prod || *num_elements != 0) {
        sem_wait(emptysem);
...
void sigHandler (int sig) {
    ...
    shmdt(num_elements);
    shmctl(segment_id7, IPC_RMID, NULL);

    shmdt(in);
    shmctl(segment_id8, IPC_RMID, NULL);

    shmdt(out);
    shmctl(segment_id9, IPC_RMID, NULL);
...
int main(int argc, char* argv[]) {
    ...
    // shared memory for num_prod
    segment_id7 = shmget(5788, sizeof(int*), IPC_CREAT | 0600);
    /** attach the shared memory segment */
    num_elements = (int*) shmat(segment_id7, NULL, 0);

    // shared memory for num_prod
    segment_id8 = shmget(5789, sizeof(int*), IPC_CREAT | 0600);
    /** attach the shared memory segment */
    in = (int*) shmat(segment_id8, NULL, 0);

    // shared memory for num_prod
    segment_id9 = shmget(5790, sizeof(int*), IPC_CREAT | 0600);
    /** attach the shared memory segment */
    out = (int*) shmat(segment_id9, NULL, 0);
